...
function deleteContact() {
  $(".delete").on('click', function(event) {
    ...
    alert('mensaje'+getParentItem);
    url = 'terceros/borro_contactos';
    $.ajax({
        async: false,
        url: BASE_URL + url,
        data: {
...
function addContact() {
  $("#btn-add").click(function() {
    ...
    if ($_emailValue == "") {
      $_getValidationField[1].innerHTML = 'Se debe completar correo electrónico';
      $_getValidationField[1].style.display = 'block';
    } else if((reg.test($_emailValue) == false)) {
      $_getValidationField[1].innerHTML = 'Email Inválido';
      $_getValidationField[1].style.display = 'block';
    } else {
    ...
    if ($_phoneValue == "") {
      $_getValidationField[2].innerHTML = 'Inválido (Debe tener 10 dígitos)';
      $_getValidationField[2].style.display = 'block';
    } else if((phoneReg.test($_phoneValue) == false)) {
      $_getValidationField[2].innerHTML = 'Por favor el Valor debe ser Númerico';
    ...
    id_tercero=$('#cab_id_tercero_i').val();
    alert(id_tercero);
    urlAlta = 'terceros/grabo_contactos';
    $.ajax({
        async: false,
        ...
        data: {
            ...
            nombre: $_nameValue,
            correo: $_emailValue,
            cargo: $_occupationValue,
            celular: $_phoneValue,
            datos: $_locationValue
        },
        type: 'POST',
        dataType: 'json',
        success: function (resp) {
              ...
              '<div class="item-content">' +
                  '<div class="user-profile">' +                     
                      ...
                      '<div class="user-meta-info">' +
                          '<p class="user-id" data-id='+ $_idValue +'>'+ $_idValue +'</p>' +
...
function editContact() {
  $('.edit').on('click', function(event) {
    ...
    $("#btn-edit").off('click').click(function(){
      ...
      var $_getInputId = getParent.find('#c-id');
...
getEmailInput.addEventListener('input', function() {
    ...
    if (getEmailInputValue == "") {
      $_getValidationField[1].innerHTML = 'Email Requerdio';
      $_getValidationField[1].style.display = 'block';
    } else if((reg.test(getEmailInputValue) == false)) {
      $_getValidationField[1].innerHTML = 'Email Incorrecto';
      $_getValidationField[1].style.display = 'block';
    } else {
      $_getValidationField[1].style.display = 'none';
    }
...
getPhoneInput.addEventListener('input', function() {
  ...
  if (getPhoneInputValue == "") {
    $_getValidationField[2].innerHTML = 'Número Celular Requerido';
    $_getValidationField[2].style.display = 'block';
  } else if((phoneReg.test(getPhoneInputValue) == false)) {
    $_getValidationField[2].innerHTML = 'Inválido (Introduzca 10 Digitos)';
    $_getValidationField[2].style.display = 'block';
  } else {
    $_getValidationField[2].style.display = 'none';
  }
